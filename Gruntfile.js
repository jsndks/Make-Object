...
module.exports = function(grunt) {
    ...
    grunt.initOptions({
        ...
        sg: {
            info: 'Whether to generate a styleguide.',
            type: Boolean
        }
    });
    ...
    grunt.initConfig({
        ...
        clean: {
            ...
            styleguide: ['<%= env.DIR_SRC %>/styleguide'],
            layouts: ['<%= env.DIR_SRC %>/layouts'],
            templates: ['<%= env.DIR_SRC %>/templates']
        },
        ...
        exec: {
            ...
            no_guide: {
                cmd: function() { return 'echo no guide' }
            }
        },
        ...
        includereplace: {
            layouts: {
                files: [{
                    cwd: '.',
                    src: '<%= env.DIR_SRC %>/includes/layouts/*.html',
                    dest: 'src/layouts',
                    expand: true,
                    flatten: true
                }]
            ...
            templates: {
                files: [{
                    cwd: '.',
                    src: '<%= env.DIR_SRC %>/includes/templates/*.html',
                    dest: '<%= env.DIR_SRC %>/templates',
                    expand: true,
                    flatten: true
                }]
            },
            styleguide: {
                files: [{
                    cwd: '.',
                    src: '<%= env.DIR_SRC %>/includes/hologram/*.html',
                    dest: '<%= env.DIR_SRC %>/assets/hologram_assets',
                    expand: true,
                    flatten: true
                }]
            }
        ...
        handlebars: {
            compile: {
                options: {
                    amd: true,
                    namespace: 'AQR.Templates',
                    processName: function(filePath) {
                        return filePath.replace(/^src\/assets\/templates\//, '').replace(/\.hbs$/, '');
                    }
                },
                files: {
                    '<%= env.DIR_SRC %>/assets/scripts/utils/templates.js': ['<%= env.DIR_SRC %>/assets/templates/*.hbs']
                }
            }
        },
        ...
        watch: {
            ...
            media: {
                ...
                tasks: ['media', 'styleguide', 'notify:media']
            },
            markup: {
                files: ['<%= env.DIR_SRC %>/includes/**/*.html', '<%= env.DIR_SRC %>/assets/templates/*.hbs'],
                tasks: ['statics', 'styleguide', 'markup', 'notify:markup']
            },
            styles: {
                files: [
                ...
                tasks: ['sass', 'styleguide', 'notify:styles']
            },
            scripts: {
                files: [
                ...
                tasks: ['scripts', 'styleguide', 'notify:scripts']
            }
        },
    ...
    grunt.registerTask('default', ['sass', 'statics', 'build', 'styleguide', 'watch']);
	...
	grunt.registerTask('vsdev', ['clean:dest', 'concurrent:build', 'clean:tmp']);
    ...
    // swap the following 2 lines to enable conditionl styleguide creation with a passed in option.  Defaults to auto-create styleguide
    // grunt.registerTask('styleguide', grunt.option('sg') ? ['clean:styleguide', 'exec:generate_styleguide'] : ['exec:no_guide']);
    grunt.registerTask('styleguide', ['clean:styleguide', 'exec:generate_styleguide']);
    grunt.registerTask('statics', ['clean:layouts', 'clean:templates', 'includereplace', 'handlebars']);
};
